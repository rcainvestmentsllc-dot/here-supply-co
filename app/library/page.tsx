...
const modules = [
  {
    ...
    alt: "A brass compass on weathered wood for The Sanctuary lesson",
    previewTitle: "Lesson 1.1 · The Sanctuary",
    previewSubtitle: "Stop carrying everything in your head.",
  },
  {
    number: "02",
    name: "Lead",
    ...
    previewTitle: "Lesson 2.1 · The Emotional Thermostat",
    previewSubtitle: "Carry pressure without handing it to the room.",
  },
  {
    number: "03",
    name: "Keep",
    ...
    previewTitle: "Lesson 3.1 · The Third Place",
    previewSubtitle: "Build a life that is bigger than work.",
  },
];
...
export default function Library() {
  return <main id="main-content" className="site"><JsonLd data={coreProduct} /><Header />
    ...
    <section id="curriculum" className="curriculum-section"><div className="curriculum-intro"><p className="section-label">THE FULL CORE CURRICULUM</p><h2>Return.<br /><em>Lead.</em><br />Keep.</h2><p>Nine lessons across three movements. Each one gives a recurring problem a name and a practice you can actually use. The three preview cards below show the opening lesson in each movement.</p></div><div className="curriculum-modules">{modules.map((module) => <article className="curriculum-module" key={module.name}><div className="curriculum-preview"><img src={module.image} alt={module.alt} width="2400" height="1260" loading="lazy" decoding="async" /><div className="curriculum-preview-copy"><span>{module.previewTitle}</span><strong>{module.previewSubtitle}</strong></div></div><div className="module-copy"><span>{module.number} · {module.name.toUpperCase()}</span><h3>{module.line}</h3><ol>{module.lessons.map((lesson) => <li key={lesson.title}><strong>{lesson.title}</strong><small>{lesson.detail}</small></li>)}</ol></div></article>)}</div></section>
