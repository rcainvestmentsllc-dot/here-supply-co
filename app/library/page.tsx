...
export default function Library() {
  return <main id="main-content" className="site"><JsonLd data={coreProduct} /><Header />
    ...
    <section id="curriculum" className="curriculum-section">
      ...
      <div className="curriculum-modules">{modules.map((module) => <section className="curriculum-module" key={module.name}>
        ...
        <div className="curriculum-lesson-grid">{module.lessons.map((lesson) => <article className="curriculum-lesson" key={lesson.slug}>
          <figure><img src={lesson.artImage} alt={lesson.artAlt} width="1672" height="942" loading="lazy" decoding="async" /><figcaption>{lesson.number} · {lesson.movement}</figcaption></figure>
    ...
    <section className="offer-section"><div className="offer-intro"><p className="section-label">ONE CLEAR PATH</p><h2>Start where the<br /><em>problem is real.</em></h2><p>The free weekly guide helps you and your wife see the week together. Focus interrupts the checking reflex. Core brings the complete practice together.</p></div><div className="offer-stack"><article className="offer-card free-offer"><span>01 · START FREE</span><h3>The free weekly guide</h3><p>A printable 15-minute home meeting for the week ahead.</p><Link href="/sunday-board#get-board">Get the free weekly guide <b>→</b></Link></article><article id="focus" className="offer-card focus-offer"><span>02 · $29</span><h3>Focus Protocol</h3><p>A guided 72-hour reset for the man who wants to choose what gets his attention again.</p><strong>$29 <small>one time</small></strong><p className="offer-note">A browser-based visual guide containing four practical moves. Immediate private access and a 14-day refund window.</p><Link href="/focus">See Focus Protocol <b>→</b></Link></article><article id="core" className="offer-card core-offer"><span>03 · $99 FOUNDING EDITION</span><h3>Iron Compass Core</h3><p>A private visual lesson library for attention, pressure, home, marriage, family connection, friendship, and the practices that bring them together.</p><strong>$99 <small>one time</small></strong><p className="offer-note">Nine visual lessons and the Core Workbook. Private access at your own pace with a 14-day refund window. The price will rise after the founding edition has real buyer feedback.</p>{coreCheckoutReady ? <a href={CHECKOUT.core}>Get Iron Compass Core <b>→</b></a> : <Link href="/sunday-board#get-board">Start free while checkout opens <b>→</b></Link>}</article></div></section>
    ...
    <section className="core-closing"><p className="section-label">START HERE</p><h2>Do not buy a better version<br />of yourself. <em>Practice being here.</em></h2><p>Start with the free weekly guide. If digital distraction is the clearest problem, Focus Protocol is the recommended first purchase.</p><Link className="button primary" href="/sunday-board">Get the free weekly guide <span>→</span></Link></section>
    <Footer />
  </main>;
}
