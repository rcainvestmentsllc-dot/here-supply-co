...
export default function Library() {
  return <main id="main-content" className="site"><JsonLd data={coreProduct} /><Header />
    <section className="core-hero">
      <div><p className="kicker">WHEN YOUR PHONE IS TAKING MORE THAN IT GIVES</p><h1>Your phone is a tool.<br /><em>You are not.</em></h1><p>A private course for two people who are tired of being in the same room and somewhere else. It starts with three days of getting the phone out of the way, then gives you nine practices for the ordinary moments where attention actually gets lost. Kids or no kids. One of you can start it alone.</p><div className="hero-buttons">{coreCheckoutReady ? <a className="button primary" href={CHECKOUT.core}>Get All the Way Here · $99 <span>→</span></a> : <Link className="button primary" href="/sunday-board#get-board">Start free while checkout opens <span>→</span></Link>}<a className="text-link" href="#curriculum">See what is inside <span>↓</span></a></div><small>{coreCheckoutReady ? "Founding-edition price. One-time purchase. Private access. 14-day refund window." : "The paid checkout is not open yet. The free meeting guide is available now."}</small></div>
    ...
    <section className="core-promise"><p className="section-label">THE PROMISE</p><div><h2>Not more information.<br />A way to <em>come back.</em></h2><p>When your attention is gone, pressure is choosing your answers, and the person across from you is getting the leftovers, advice is not enough. This course gives the problem a name, the practice a place, and the week a shape you can actually keep.</p></div></section>

    <section className="included-section"><div className="included-heading"><p className="section-label">WHAT YOU ACTUALLY GET</p><h2>Name the problem.<br />Work the <em>practice.</em></h2><p>The course opens with a three day attention reset, then moves through nine lessons across attention, steadiness under pressure, your marriage, your kids if you have them, and the parts of a life worth protecting.</p></div><div className="included-list"><article><span>01</span><h3>The three day reset</h3><p>Four moves, plus what the seventy two hours actually feel like hour by hour and the exact sentences your head will use on day five to talk you out of it.</p></article><article><span>02</span><h3>Nine visual lessons</h3><p>Each focused lesson names a recognizable problem and gives you one concrete practice to use in real life.</p></article><article><span>03</span><h3>A real workbook</h3><p>Download and print 16 useful pages, or type privately in the browser version that saves on your device.</p></article></div></section>
