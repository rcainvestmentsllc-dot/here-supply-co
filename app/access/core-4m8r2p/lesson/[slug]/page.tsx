...
export const metadata: Metadata = {
  title: "Iron Compass Core Lesson",
...
export default async function CoreLessonPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main-content" className={`lesson-shell lesson-${lesson.movement.toLowerCase()}`}>
    <header className="access-header lesson-header"><Link href="/access/core-4m8r2p">← CORE LIBRARY</Link><span>{lesson.number} · {lesson.movement}</span></header>
    ...
    <figure className="lesson-art">
      <img src={lesson.artImage} alt={lesson.artAlt} width="1672" height="942" />
    ...
    <figure className="lesson-detail-art">
      <img src={lesson.detailImage} alt={lesson.detailAlt} width="1672" height="942" loading="lazy" decoding="async" />
      <figcaption><span>THE PRACTICE IN REAL LIFE</span><p>{lesson.detailCaption}</p></figcaption>
    </figure>
    ...
    <nav className="lesson-next" aria-label="Lesson navigation">
      ...
      {next ? <Link href={`/access/core-4m8r2p/lesson/${next.slug}`}><small>NEXT · {next.number}</small><strong>{next.title} →</strong></Link> : <Link href="/access/core-4m8r2p/workbook"><small>NEXT</small><strong>Core Workbook →</strong></Link>}
    </nav>
  </main>;
}
